		...
		def get_pending_requests(self, event, context):
				...
				try:
						...
						if reviewerGroup in groups:
								...
								result = "The idToken for " + requester + " does not contain the " + reviewerGroup + " group"
								print(result)
								status_code = 400
				except Exception as error:
		...
		def create_request(self, event, context):
				...
				try:
						...
						if input_body:
								...
								print("Auto approval options enabled: " + auto_approve)
								now = datetime.now().strftime('%x %X')
								request = {
										'id': id,
										'requester': requester,
										...
										'request_status': 'Approved' if auto_approve else 'Requested',
										'request_time': request_time,
										'expiration_time': (datetime.strptime(now, '%x %X') + timedelta(minutes=int(request_duration))).strftime('%x %X') if auto_approve else '',
										'review_time': datetime.now().strftime('%x %X') if auto_approve else '',
										'reviewer': requester if auto_approve else ''
								}
								dynamodb = boto3.resource('dynamodb')
								dbname = os.environ['db_table']
								table = dynamodb.Table(dbname)
								...
								status_code = 200
								# trigger the approval if enabled 
								
						

								if(bool(auto_approve)):
									print("Yes it is bool: ", str(bool(auto_approve)))

								if (auto_approve):
									status_code = self.approve_request_internal(table,request_time,request_duration,bool(auto_approve),requester,id,context)
